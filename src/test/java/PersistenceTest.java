...
public class PersistenceTest {
    ...
    void test8() {
        ...
        try {
            ...
            Memo memo = em.find(Memo.class, 4); // 1. 조회를 해온다.
            System.out.println("memo.getId() = " + memo.getId());
            System.out.println("memo.getUsername() = " + memo.getUsername());
            System.out.println("memo.getContents() = " + memo.getContents());

            System.out.println("\n수정을 진행합니다.");
            memo.setUsername("Update");         //2. 변경할 데이터를 넣어준다.
            memo.setContents("변경 감지 확인");    //2.

            System.out.println("트랜잭션 commit 전");
            et.commit();                        //3. commit하면 DB에 반영이 된다.
            System.out.println("트랜잭션 commit 후"); // 언제 비교하냐면 트랜잭션 commit호출되면서 fllush 호출되면서 비교를 한다.
